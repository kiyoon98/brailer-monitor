...
- 8초 이내 구간 병합
- 위치/크기/하단 대형 바다 영역/세로형 빈 그물/3-4초 정지/시간 고립 및 1초 이하 3프레임 burst/균일한 밝은색 및 색상 이상 탐지 제거 후처리. 균일한 밝은색은 bbox의 평균 밝기 170 이상, 중앙 밝기 190 이상이고 픽셀의 72% 이상이 밝기 160 이상이면서 중앙 Lab 색상과의 거리가 35 이내일 때로 판정합니다. 같은 카메라의 12시간 작업 범위 안에 크기와 위치가 유사한 다른 전재가 있으면 위치·크기·상대적 색상 이상·시간 고립 제거에서 해당 탐지를 보호하지만, 균일한 밝은색은 반복 위치와 관계없이 제거합니다. 단, 작업 전체에서 위치 이상인 탐지가 중심 이동 5px 이하, bbox 폭·높이 변화 10% 이하로 3개 이상 영상에 걸쳐 6회 이상, 2분 이상 반복되면 장시간 고정 배경으로 제거합니다. 또한 중심이 화면 하단 70% 아래이면서 활성 탐지 ROI 좌우 경계 2% 이내이거나 중심이 화면 하단 80% 아래인 유사 탐지가 3개 이상 영상에서 반복되면 고정 배경 오탐으로 보고 위치 반복 보호를 적용하지 않습니다. 하단 대형 바다 영역은 작업 중앙 mask 면적의 4배를 넘고 bbox가 화면 하단 98%에 닿으며 중심이 하단 65% 영역에 있을 때 제거되고 반복 탐지 보호를 적용하지 않습니다.
- 현재 탐지 결과 저장
- 저장된 결과 불러오기
- 외부 문서용 리포트 생성
